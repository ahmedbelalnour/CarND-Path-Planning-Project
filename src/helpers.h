...
bool is_target_lane_safe(const int target_lane, const int prev_size, const double car_s, const std::vector<std::vector<double>> &sensor_fusion)
{
	...
	for (int lane_index = 0; lane_index < sensor_fusion.size(); lane_index++)
	{
		...
		if (d < (2 + 4 * target_lane + 2) && d >(2 + 4 * target_lane - 2))
		{
			...
			std::cout << "car_s: " << car_s << ", check_car_s: "<< check_car_s << std::endl;
			if (check_car_s > car_s)
			{
				std::cout << "Forward distance: " << (check_car_s - car_s) << std::endl;
			}
			if ((check_car_s > car_s) && ((check_car_s - car_s) < FORWARD_SAFE_DISRANCE))
			{
			...
			{
				std::cout << "Backward distance: " << (car_s - check_car_s) << std::endl;
			}
			if ((car_s > check_car_s) && ((car_s - check_car_s) < BACKWARD_SAFE_DISRANCE))
			{
				// Then we need to take some actions: like lower the speed so we don't crash in to the infront of us
				// We can raise a flag to try to change the lane
